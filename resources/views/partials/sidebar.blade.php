<aside id="sidebar" class="fixed inset-y-0 left-0 z-30 hidden bg-brand-900 text-white lg:block">
    ...
    <nav class="space-y-1 p-4">
        ...
        @foreach ($links as $link)
            <a href="{{ route($link['route'], [], false) }}"
               class="block whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-medium transition {{ request()->routeIs($link['route']) ? 'bg-brand-700 text-white' : 'text-blue-100 hover:bg-brand-700/70' }}">
                {{ $link['label'] }}
            </a>
        @endforeach
    </nav>
